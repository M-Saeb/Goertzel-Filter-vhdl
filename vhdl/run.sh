...
vlib work && vcom '-2019' '-o' read_signal.vhd design.vhd testbench.vhd  && vsim -c -do "vsim goertzel_filter; run -all; exit"  ; echo 'Creating result.zip...' && zip -r /tmp/tmp_zip_file_123play.zip . && mv /tmp/tmp_zip_file_123play.zip result.zip